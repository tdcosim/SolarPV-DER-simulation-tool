...
class TestPVDER(unittest.TestCase):
	...
	test_scenarios = {'default':{'LVRT_ENABLE':True,'tEnd':5.0,'Vnominal':1.0,'Vfault':0.8},
					  'LVRT1':{'LVRT_ENABLE':True,'tEnd':10.0,'tfault_start':4.0,'tfault_duration':0.5,'Vfault':0.7},
					  'LVRT2':{'LVRT_ENABLE':True,'tEnd':10.0,'tfault_start':4.0,'tfault_duration':2.0,'Vfault':0.7},
					  'LVRT3':{'LVRT_ENABLE':True,'tEnd':10.0,'tfault_start':4.0,'tfault_duration':2.0,'Vfault':0.7}}
	...
	def setup_simulation(self,scenario='default'):
		...
		for i in range(self.n_instances):
			...
			if SinglePhase:
				self.DER_model_list.append(SolarPVDERSinglePhase(events=self.events_list[-1],configFile=config_file,**kwargs))
			else:
				self.DER_model_list.append(SolarPVDERThreePhase(events=self.events_list[-1],configFile=config_file,**kwargs))
	...
	def specify_scenario(self,events,DER_model,sim,scenario='LVRT1'):
		...
		Vfault = self.return_settings(scenario=scenario,parameter='Vfault',settings_type='test')
		
		tfault_start = self.return_settings(scenario=scenario,parameter='tfault_start',settings_type='test')
		tfault_duration = self.return_settings(scenario=scenario,parameter='tfault_duration',settings_type='test')
		...
		if tfault_start is not None:
		   events.add_grid_event(tfault_start,Vfault)
		
		if tfault_duration is not None:
		   events.add_grid_event(tfault_start+tfault_duration,Vnominal)	
